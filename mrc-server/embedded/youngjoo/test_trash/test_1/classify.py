...
def run(model: str, max_results: int, score_threshold: float, num_threads: int,
 ...
 # Continuously capture images from the camera and run inference
  while True:
    ...
    # Capture the image
    numpy_image = picam2.capture_array()
    ...
    for idx, category in enumerate(categories.classifications[0].categories):
      ...
      cv2.putText(numpy_image, result_text, text_location, cv2.FONT_HERSHEY_PLAIN,
    ...
    if counter % _FPS_AVERAGE_FRAME_COUNT == 0:
      ...
      cv2.putText(numpy_image, fps_text, text_location, cv2.FONT_HERSHEY_PLAIN,
                  _FONT_SIZE, _TEXT_COLOR, _FONT_THICKNESS)

    # Display the image
    cv2.imshow('image_classification', numpy_image)
    if cv2.waitKey(1) == 27:
      break
